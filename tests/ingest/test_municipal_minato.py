...
@pytest.mark.asyncio
async def test_municipal_minato_pipeline(
    monkeypatch: pytest.MonkeyPatch,
    ...
    await _ensure_equipments(
        session,
        ["treadmill", "upright-bike", "chest-press", "shoulder-press"],
    )
    ...
    assert urls == {index_url, facility_url, notice_url, pdf_url}

    pdf_page = next(page for page in pages if page.url == pdf_url)
    assert pdf_page.response_meta.get("content_type") == "application/pdf"
    assert "トレッドミル" in (pdf_page.raw_html or "")
    ...
    assert meta_map[notice_url]["municipal_page_type"] == "article"
    assert meta_map[pdf_url]["municipal_page_type"] == "article"

    await parse.parse_pages("municipal_minato", limit=10)
    await normalize.normalize_candidates("municipal_minato", limit=10)
    ...
    notice_candidate = by_url[notice_url]
    pdf_candidate = by_url[pdf_url]

    assert facility_candidate.pref_slug == "tokyo"
    assert facility_candidate.city_slug == "minato"
    payload = facility_candidate.parsed_json or {}
    assert payload["equipments_slotted"] == [
        {"slug": "treadmill", "count": 6},
        {"slug": "upright-bike", "count": 4},
        {"slug": "chest-press", "count": 2},
        {"slug": "shoulder-press", "count": 1},
    ]
    assert payload["meta"]["canonical_hint"] == "高輪区民センター トレーニングルーム"

    assert notice_candidate.parsed_json["meta"]["create_gym"] is False
    assert pdf_candidate.parsed_json["meta"]["create_gym"] is False
    pdf_payload = pdf_candidate.parsed_json or {}
    assert "トレッドミル" in " ".join(pdf_payload.get("equipments_raw", []))
    assert pdf_payload["page_type"] == "article"
