...
export function SearchFiltersContent({
  ...
  return (
    <form
      className={cn(
        "flex h-full flex-col gap-6 rounded-3xl border border-border/80 bg-card/95 p-5 shadow-sm backdrop-blur supports-[backdrop-filter]:bg-card/80 sm:p-7",
        className,
      )}
      onSubmit={event => {
    ...
    >
      ...
      <div className="space-y-3">
        ...
        <div className="grid gap-2">
          ...
          <p className="text-xs text-muted-foreground" id={distanceHelpTextId}>
            {!hasLocation
              ? "位置情報を設定すると検索範囲を調整できます。"
              : "基準点からの検索半径を指定します。"}
          </p>
        </div>
      </div>
